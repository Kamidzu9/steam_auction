  ...
  return (
    <div className="space-y-6">
      <section className="surface rounded-2xl p-6">
        ...
        <p className="text-muted mt-1 text-sm">Spiele: {games.length}</p>
      ...
      <section className="surface rounded-2xl p-6">
        <h2 className="font-display text-lg text-white">Pool-Spiele</h2>
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-300 md:grid-cols-2">
            {pool.picks.map((pick) => (
              <div key={pick.id} className="rounded-xl border border-white/10 bg-black/30 px-3 py-2">
                <div className="text-white">{pick.game?.name ?? "Unbekannt"}</div>
