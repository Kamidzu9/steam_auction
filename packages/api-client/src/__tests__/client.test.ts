...
describe("ApiClient methods", () => {
  ...
  it("refresh sends POST /auth/refresh", async () => {
    const mockTokens = { accessToken: "tok", expiresIn: 900, user: { id: "u1" } };
    fetchMock.mockResolvedValue(new Response(JSON.stringify(mockTokens), { status: 200 }));
  ...
  it("getPools sends GET /pools", async () => {
    fetchMock.mockResolvedValue(new Response(JSON.stringify({ pools: [] }), { status: 200 }));
  ...
  it("createPool sends POST /pools with body", async () => {
    ...
    fetchMock.mockResolvedValue(new Response(JSON.stringify({ pool: mockPool }), { status: 200 }));
    await client.createPool({ friendId: "f1", name: "My Pool" });
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe("http://api.example.com/pools");
    expect(init.method).toBe("POST");
    expect(JSON.parse(init.body as string)).toMatchObject({ friendId: "f1", name: "My Pool" });
  });

  it("pickFromPool sends POST /pools/:id/pick", async () => {
    const mockPick = { pick: { id: "g1", appId: 730, name: "CS2", storeUrl: "" } };
    fetchMock.mockResolvedValue(new Response(JSON.stringify(mockPick), { status: 200 }));
  ...
  it("getRecentPicks sends GET /pools/:id/recent-picks with limit", async () => {
    fetchMock.mockResolvedValue(new Response(JSON.stringify({ appIds: [730] }), { status: 200 }));
    const result = await client.getRecentPicks("pool-1", 5);
    expect(result.appIds).toEqual([730]);
    const [url] = fetchMock.mock.calls[0] as [string];
    expect(url).toBe("http://api.example.com/pools/pool-1/recent-picks?limit=5");
  });

  it("addGameToPool detects skipped response", async () => {
    fetchMock.mockResolvedValue(
      new Response(JSON.stringify({ skipped: true, reason: "forbidden_word" }), { status: 200 })
    );
    const result = await client.addGameToPool("pool-1", { appId: 999, name: "Bad Game" });
